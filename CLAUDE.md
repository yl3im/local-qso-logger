...
- **Satellite fields** (`SAT_NAMES`, `SAT_MODES`): when `q.propMode === "SAT"`, the QSO form reveals five satellite-only fields via a `.is-sat` class on `#qso-form` (CSS: `.sat-only { display: none }`, `.qso-form.is-sat .sat-only { display: flex }`). Fields: `SAT_NAME` (dropdown, ~110 entries), `BAND_RX` (dropdown, same options as `BAND` with empty first), `SAT_MODE` (dropdown of AMSAT mode letters A/B/J/K/L/R/S/T/U/V/W/X, each entry is `[code, uplinkBand, downlinkBand]`), `GRIDSQUARE` (other station's Maidenhead), `MY_GRIDSQUARE` (operator's Maidenhead). Selecting a `SAT_MODE` auto-adjusts `BAND` = uplink and `BAND_RX` = downlink. **Satellite properties (`q.satName / q.bandRx / q.satMode / q.gridSquare / q.myGridSquare`) are only attached to the QSO when `propMode === "SAT"` — non-satellite QSOs don't carry these keys at all.** Editing a QSO from sat to non-sat (or vice versa) adds or deletes the properties as needed. On import, sat properties are only attached when the source ADIF record actually contained them. Legacy imported `SAT_MODE` values (like "U/V") not in the enum are preserved via a one-off `<option>` appended when the QSO is opened for edit.
...
- **ADIF export** (`buildAdif`/`adifField`): declares `ADIF_VER 3.1.7` (via `ADIF_VERSION`), `PROGRAMID local-qso`, `PROGRAMVERSION` (from `APP_VERSION`), and `CREATED_TIMESTAMP` (UTC `YYYYMMDD HHMMSS` via `nowAdifTimestamp`). Per-QSO maps `CALL/QSO_DATE/TIME_ON/BAND/MODE/SUBMODE/RST_SENT/RST_RCVD`. SUBMODE emitted only when the stored mode is a submode. After the typed fields, every entry in `q.extras` is re-emitted verbatim — that's how the app round-trips ADIF fields it doesn't model in its UI (COMMENT/NAME/GRIDSQUARE/FREQ/DXCC/QSL_*/etc.). Field length in the tag is byte length via `TextEncoder`. Downloads via Blob.
